...
@attacker_registry.register("BadNets", ["data_poisoning"], "attacker")
...
class BadNets(DPBase, Client):
    ...
    def __init__(self, *args, trigger_size=5, attack_model="all2one", poisoning_ratio=0.32, target_label=6, source_label=1, attack_strategy="continuous", single_epoch=0, poison_frequency=5, attack_start_epoch=None, **kwargs):
        ...
        self.default_attack_params = {
            'trigger_size': trigger_size, "attack_model": attack_model, "poisoning_ratio": poisoning_ratio, "target_label": target_label, "source_label": source_label, "attack_strategy": attack_strategy, "single_epoch": single_epoch, "poison_frequency": poison_frequency, "attack_start_epoch": attack_start_epoch}
...
@attacker_registry.register("BadNets_image", ["data_poisoning"], "attacker")
@actor('attacker', 'data_poisoning')
class BadNets_image(DPBase, Client):
    ...
    def __init__(self, *args, trigger_path="./attackers/triggers/trigger_white.png", trigger_size=5, attack_model="all2one", poisoning_ratio=0.32, target_label=6, source_label=1, attack_strategy="continuous", single_epoch=0, poison_frequency=5, **kwargs):
        super().__init__(*args, **kwargs)
        self.default_attack_params = {"trigger_path": trigger_path, "trigger_size": trigger_size, "attack_model": attack_model,
                                      "poisoning_ratio": poisoning_ratio, "target_label": target_label, "source_label": source_label, "attack_strategy": attack_strategy, "single_epoch": single_epoch, "poison_frequency": poison_frequency}
        self.update_and_set_attr()
        self.define_synthesizer()
        poison_epochs = self.generate_poison_epochs(
