...
class Worker:
    ...
    def get_dataloader(self, dataset, train_flag=True, poison_epochs=None, **kwargs):
        """
        ...
        # Assuming self.poisoned_set exists for attackers, otherwise use dataset
        data = self.poisoned_set[1 - train_flag] if poison_epochs and hasattr(self, 'poisoned_set') else dataset
        
        dataloader = torch.utils.data.DataLoader(
            data,
            batch_size=self.args.batch_size,
            shuffle=train_flag,
            num_workers=getattr(self.args, 'num_workers', 0), # Use num_workers from args, default to 0
            pin_memory=False  # Set to False for CPU consistency
        )
        return dataloader

    def criterion_fn(self, y_pred, y_true, reduction='mean', **kwargs):
        return torch.nn.CrossEntropyLoss(reduction=reduction)(y_pred, y_true)
        ...
        with torch.no_grad():
            for images, targets in test_loader:
                images, targets = images.to(
                    self.args.device), targets.to(self.args.device)
                pred_probs = model(images)
                loss = self.criterion_fn(pred_probs, targets, reduction='sum') # Use sum reduction for accumulation
                predicted = torch.argmax(pred_probs.data, 1)
                num_samples += len(targets)
                overall_correct += (predicted == targets).sum().item()
                test_loss += loss.item() # loss.item() is now the sum for the batch
