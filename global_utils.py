...
def setup_logger(logger_name, log_dir, level=logging.INFO, stream=False):
    ...
    fileHandler = logging.FileHandler(log_file_path, mode='w', encoding='utf-8')
...
class Register(dict):
    ...
    def register(self, name, attributes, category="default"):
        def decorator(cls):
            cls._category = category
            cls._attributes = attributes
...
class TimingRecorder:
    def __init__(self, id, output_file):
        ...
        self.global_timings = {}
        # Construct the directory for time logs within the main output directory
        time_log_dir = os.path.join(output_file, "time_logs")
        # Make logger name unique for TimingRecorder as well
        logger_name = f"TimingRecorder_{id}_{int(time.time())}"
        self.logger = setup_logger(
            logger_name, time_log_dir, level=logging.INFO) # Pass the directory
